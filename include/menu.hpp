...
class Menu {
 public:
  Menu(const std::vector<std::string>& opts) noexcept
      : m_opts(opts),
        m_idx(0),
        m_opt_fn([](int, const std::string& opt) { return opt; }),
        m_opt_enter_fn([](int, const std::string&) { return true; }) {}

  void on_opt_show(std::function<std::string(int, const std::string&)>&& fn) noexcept {
    m_opt_fn = fn;
  }
  ...
  void show() noexcept {
    ...
    std::cout << "\n\n\n\nuse `w`, `s`, `a`, `d` to navigate, `enter` to enter, `q` to quit.";
  }
